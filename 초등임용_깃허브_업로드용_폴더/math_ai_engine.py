...
MATH_MATRIX_PATH = os.path.join(BASE_DIR, "data", "math_gaknon_matrix.json")
MATH_THEORY_PATH = os.path.join(BASE_DIR, "data", "math_theory_base.json")
MATH_EXAM_PATH = os.path.join(BASE_DIR, "data", "math_exam_db.json")

try:
    with open(MATH_MATRIX_PATH, "r", encoding="utf-8") as f:
        MATH_MATRIX = json.load(f)
except Exception:
    MATH_MATRIX = {}

try:
    with open(MATH_THEORY_PATH, "r", encoding="utf-8") as f:
        MATH_THEORY = json.load(f)
except Exception:
    MATH_THEORY = {}
...
def extract_json(text: str):
    """Robustly extracts and parses JSON from text, handling markdown fences and surrounding commentary."""
    text = text.strip()
    try:
    ...
    raise ValueError(f"AI 응답에서 유효한 JSON 데이터를 파싱할 수 없습니다:\n{text[:250]}")
...
def test_api_connection(api_key: str, model_name: str = None) -> tuple[bool, str, str]:
    """Tests API connection with a lightweight probe."""
    if not api_key or not api_key.strip():
    ...
    for m in models_to_try:
        try:
            ...
            if resp and resp.text:
                return True, f"정상 연결됨 ({m} 동작 확인)", m
        except Exception as e:
            last_err = str(e)
            continue
...
def generate_math_ai_questions(api_key: str, q_type: str, tier: str, domain: str, count: int, model_name: str = None):
    client = get_gemini_client(api_key)
    ...
    context_data = MATH_MATRIX.get("abcd_gaknon_matrix", {})
    ...
    if domain and domain != "전체":
        domain_filtered = [item for item in tier_items if domain in str(item.get("domain", "")) or domain in str(item.get("topic", ""))]
        if domain_filtered:
    ...
    random_seed_str = f"수학출제세션-{int(time.time()*1000) % 100000}-{random.randint(100, 999)}"

    system_instruction = f"""
당신은 대한민국 '초등교원 임용후보자 선정경쟁시험(초등 임용고시)' 1차 교육과정 수학과의 최고 권위 수석 출제위원입니다.
반드시 제공된 [초등 수학 1682p 각론, 14개년 기출 DB, 2022 개정 수학과 교육과정, 수학교육론]을 바탕으로, 실제 임용고시의 엄격한 출제 기준과 루브릭에 맞추어 완전히 새롭고 참신한 변형 문제를 생성해야 합니다.
...
3. 세부 영역: {domain}
4. 생성할 문항 수: {count}개
5. 매번 완전히 다른 수학적 조작 활동(수직선, 영역모델, 수모형, 전개도), 학생 오개념 대화, 교사 수업 지도안을 조합하여 중복 없는 새로운 미기출 문제를 창작하십시오.
...
[
  {{
    ...
    "dialogue": "실전 기출 대문항 세트일 때 교사 대화문 및 수업 지도안 HTML (단일 문항이면 null)",
    "sub_questions": [
      {{
        "sub_no": "1)",
        "type": "단답형 또는 서술형",
...
"""
    ...
    prompt = f"""
[무작위 수학 출제 세션 식별자]: {random_seed_str}
다음 [선별된 초등 수학 각론 및 기본이론 지식 베이스]를 참고하여 실제 한국교육과정평가원 임용고시 시험지에 처음 등장하는 신유형 변형 {count}문항을 생성해 주십시오:
...
def evaluate_math_with_ai(api_key: str, question: str, model_answer: str, rubric_list: list, user_answer: str, max_points: float, model_name: str = None):
    ...
    prompt = f"""
당신은 대한민국 초등교원 임용시험 1차 교육과정 수학과 수석 채점관입니다.
다음 문제와 세부 루브릭(채점 기준표), 공식 모범답안을 기준으로 수험생의 답안을 엄격하게 칼채점하십시오.
...
{{
  ...
  "evaluator_feedback": "수험생을 위한 2~3문장의 날카롭고 유익한 수학적 총평 피드백"
}}
"""
